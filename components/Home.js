...
function Home() {
        ...
        if (data.result) {
          dispatch(
            login({
              ...
              profil: data.profil,
              reponses: data.questionnairePerso,
            })
          );
  ...
  let modalContent = (
    <div>
      <p>{MessCB}</p>
      <button id="CLOTURE" onClick={() => closeModale()}>
        FERMER
      </button>
    </div>
  );
    ...
    <div>
      ...
      <main className={styles.main}>
        <div className={styles.prez}>
          Netus et malesuada fames ac. Sit amet volutpat consequat mauris.
          Pharetra pharetra massa massa ultricies mi quis hendrerit dolor. Vel
          elit scelerisque mauris pellentesque pulvinar pellentesque habitant
          morbi. Risus pretium quam vulputate dignissim suspendisse in est. A
          condimentum vitae sapien pellentesque habitant. Nulla malesuada
          pellentesque elit eget. Vel elit scelerisque mauris pellentesque
          pulvinar pellentesque habitant morbi. Volutpat diam ut venenatis
          tellus. Morbi tempus iaculis urna id volutpat lacus laoreet non.{" "}
        </div>
